...
namespace wUtility
{
    ...
    public partial class SimilarityDatabaseForm : Window
    {
        ...
        public bool EpirDate()
        {
            string dateNow = DateTime.Now.ToShortDateString();
            DateTime datex = Convert.ToDateTime(dateNow);
            DateTime constdate = Convert.ToDateTime("11/30/22");
            TimeSpan span = datex - constdate;
            if (span.Days > 5)
                return false;
            else
                return true;
        ...
        public bool ValidateDateIn()
        {
           using (DBHelper dbh = new DBHelper(Totalsystem.CurrentTotalsystem.TotalsystemConnectionString))
                {
                    DataTable dt = dbh.ExecuteQuery(@"select top 1 datein 
                                                  from turn 
                                                  order by datein desc");
                    PersianCalendar pc = new PersianCalendar();
                    string[] result = dt.Rows[0][0].ToString().Split('/');
                    DateTime dtime = new DateTime(int.Parse(result[0]), int.Parse(result[1]), int.Parse(result[2]), pc);
                    DateTime constdate = Convert.ToDateTime("11/30/22");
                    TimeSpan span = dtime - constdate;
                if (span.Days > 0)
                {
                    if (span.Days > 5)
                        return false;
            ...
            try
            {
                ...
                switch (DatabseModelName)
                {
                    ...
                    case "Requests":
                        ...
                        break;
                    case "Phisio":
                        FillDestenationDatabasesCmbFilterd(Phisio.CurrentPhisio.CurrentPhisioDatabseModel);
                        FillDatasourceDatabasesCmbFilterd(Phisio.CurrentPhisio.CurrentPhisioDatabseModel);
                        break;
                    case "Food":
                        FillDestenationDatabasesCmbFilterd(Food.CurrentFood.CurrentFoodDatabseModel);
                        FillDatasourceDatabasesCmbFilterd(Food.CurrentFood.CurrentFoodDatabseModel);
                        break;
                    case "Blood":
                        FillDestenationDatabasesCmbFilterd(Blood.CurrentBlood.CurrentBloodDatabseModel);
                        FillDatasourceDatabasesCmbFilterd(Blood.CurrentBlood.CurrentBloodDatabseModel);
                        break;
                }
            }
            catch(Exception ex)
            ...
            try
            {
                ...
                switch (DatabseModelName)
                {
                    ...
                    case "Den":
                        ...
                        break;
                    case "Food":
                        if (databaseModelNamelist[0].ToString() == CommanClass.DatabaseModelName.Food)
                        {
                            databaseModelNamelist = CommanClass.DestnationDatabases.DestnationDatabasesList.Where(o => o.DestnationDatabaseModel == uiDatababasesModelCmb.Text)
                                                                                  .Select(os => os.DestnationDatabaseModel).ToList();
                            databaseModelName = databaseModelNamelist[0].ToString();
                            //ConnectedDatabsesName = uiDatababaseConnected.Text;
                            connectionStringlist = Food.CurrentFood.FoodDatabasesList.Where(o => o.DatabaseName == uiDestenationDatababases.Text)
                                                                                               .Select(os => os.ConnectionStringRead).ToList();
                            connectionString = connectionStringlist[0].ToString();
                            //FillDatabaseConnectedList(Totalsystem.CurrentTotalsystem.TotalsystemConnectionString, uiDatababaseConnected.Text);
                ...
                switch (DatabseModelName)
                {
                    ...
                    case "Food":
                        if (Food.CurrentFood.CurrentFoodDatabseModel == CommanClass.DatabaseModelName.Food)
                        {
                            ...
                            Task task = Task.Run(async () => await GetDirectoryAsync(Food.CurrentFood.FoodConnectionString, ConnetDatabaseName, false));
                            Task.WaitAll(task);
                            DataTable dataTable = DatasourceDatabasesDt;
                            uiDatasourceDatabaseGrid.ItemsSource = dataTable;
                            DatasourceDatabseConnectionString = Food.CurrentFood.FoodConnectionString;
                        }
                        //if (Food.CurrentFood.CurrentFoodDatabseModel == CommanClass.DatabaseModelName.Food)
        ...
        public void CreateScripts(List<DataRow> discordListRow,string connectionString,bool isDatasourceDatabase)
        {
            using (DBHelper dbh = new DBHelper(connectionString))
            {
                ...
                if (isDatasourceDatabase)
                {
                    ...
                    dataTable2 = DestenationDatabasesDt;
                }
                else
                {
                    dataTable = DestenationDatabasesDt;
                    dataTable2 = DatasourceDatabasesDt;
                }
                ...
                foreach (var item in discordListRow)
                {
                    ...
                    if (dataType == "char" || dataType == "varchar" || dataType == "nvarchar" || dataType == "nchar" /*|| dataType == "text"*/)
                    {
                    ...
                    else if (dataType == "decimal" || dataType == "numeric")
                    {
                        ...
                        if (createOrAlter != "CREATE")
                            cOLUMN_DEFAULT = 0.ToString() + withvalues;
                        else
                    ...
                    else if (dataType == "datetime" || dataType == "datetime2")
                    {
                        script = script + ' ' + dataType +' ';// + "(" + nUMERIC_PRECISION + ", " + nUMERIC_SCALE + ")" + ' ';
                        if (createOrAlter != "CREATE")
                            cOLUMN_DEFAULT = "GetDate()" + withvalues;
                        else
                     ...
                     if((isConstraintName || resultAnyCONSTRAINT_NAME) && !string.IsNullOrEmpty(cONSTRAINT_NAME))
                    {
                     ...
                     if (!string.IsNullOrEmpty(cONSTRAINT_NAME))
                    {
                        ...
                        if(!hasPrimaryKey)
                            hasPrimaryKey = dataTable2.AsEnumerable().Where(o => o["Constraint_Type"].ToString() == item["Constraint_Type"].ToString()
                                                                              && o["TABLE_NAME"].ToString() == item["TABLE_NAME"].ToString()).Any();

                        var primaryKeyItems = DiscordResultExecutedList.AsEnumerable().Where(o => o["Constraint_Type"].ToString() == item["Constraint_Type"].ToString()
                                                                              && o["TABLE_NAME"].ToString() == item["TABLE_NAME"].ToString()).ToList();
                        if (hasPrimaryKey && primaryKeyItems.Count>0)
                        {
                    ...
                    else
                    {
                        ...
                        if(((isConstraintName || resultAnyCONSTRAINT_NAME) && !string.IsNullOrEmpty(constraintQuery)) || hasPrimaryKey 
                            && !string.IsNullOrEmpty(dropConstraintQuery)) 
